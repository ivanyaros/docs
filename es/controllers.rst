...
Los controladores corresponden a la C  en el sistema MVC(ModeloVistaControlador).
Una acción de su controlador es envocada después de haber aplicado el enrutamiento
Y encontrado el controlador  correcto.
su controlador debería encargarse de interpretar los datos de la petición, asegurando que 
Se carguen los modelos correctos y renderizar la respuesta o vista corresppndiente.
Your controller
should handle interpreting the request data, making sure the correct models
are called, and the right response or view is rendered. Los controladores son como intermediarios entre los modelos y ls vistas
Controllers can be
thought of as middle layer between the Model and View. Le interesa mantener los controladores pequeños(poco código) y los modelos gordos.You want to keep your
controllers thin, and your models fat. Esto le permitirá reutilizar el código y será mas facil testear la aplicación. This will help you reuse
your code and makes your code easier to test.

De normal, un controlador se encargs de la lógica de un modelo.Commonly, a controller is used to manage the logic around a single model. Por ejemplo, si estuviera haciendo un sitio web para un horno online,For
...
.. meta::
    ...
    :keywords lang=en: correct models,controller class,controller controller,core library,single model,request data,middle man,bakery,mvc,attributes,logic,
trollers
###########
